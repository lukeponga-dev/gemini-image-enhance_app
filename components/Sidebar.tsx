...
import { 
    GeneratorIcon, EditorIcon, RemoveIcon, StyleIcon, GalleryIcon, SparklesIcon, AnalystIcon, ChatIcon, CloseIcon,
    UnblurIcon, Upscale8KIcon, RestoreIcon, BgRemoverIcon, UpscaleIcon
} from './Icons';

export type Mode = 'generate' | 'edit' | 'remove' | 'style' | 'enhancer' | 'gallery' | 'analyst' | 'chat' | 'unblur' | 'upscale8k' | 'restore' | 'bg-remover';
...
const appFeatures: Tool[] = [
  ...
  { id: 'chat', label: 'AI Chatbot', icon: ChatIcon },
  ...
  return (
    <>
      <div 
        ...
        aria-hidden="true"
      />
      <aside className={`fixed top-0 h-full w-64 bg-zinc-950/80 backdrop-blur-xl border-r border-zinc-800/80 z-40 flex flex-col transition-transform transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 md:flex`}>
        <div className="flex items-center justify-between p-4 border-b border-zinc-800/80">
           <div className="flex items-center space-x-3">
             ...
             <h2 className="text-lg font-bold text-white tracking-tight">AI Picz Enhancer</h2>
           </div>
           <button onClick={onClose} className="md:hidden p-1 text-zinc-400 hover:text-white" title="Close menu">
        ...
        <nav className="flex-1 p-2 space-y-2 overflow-y-auto no-scrollbar">
            ...
            <div className="p-2">
                <h3 className="px-4 py-2 text-xs font-semibold text-zinc-500 uppercase tracking-wider">AI Tools</h3>
                {renderToolList(aiTools)}
            </div>
             <div className="p-2">
                <h3 className="px-4 py-2 text-xs font-semibold text-zinc-500 uppercase tracking-wider">App Features</h3>
