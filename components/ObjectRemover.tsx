  ...
  return (
    <div className="w-full max-w-5xl mx-auto">
      <div className="text-center mb-8">
        <h2 className="text-3xl font-bold text-white tracking-tight">AI Object Remover</h2>
        <p className="mt-2 text-lg text-slate-400">Erase unwanted objects, people, or text from your photos.</p>
      </div>
      ...
      {originalImage && (
        <div className="bg-slate-900/50 rounded-2xl p-4 sm:p-6 md:p-8 border border-slate-800 shadow-2xl shadow-black/30">
          {!resultImage && !isLoading ? (
             <div className="grid md:grid-cols-2 gap-8 items-center">
               ...
               <div className="flex flex-col items-center justify-center h-full">
                  <p className="text-slate-400">The result will appear here.</p>
               </div>
             </div>
          ) : (
              ...
              {resultImage && (
               ...
               {isLoading && (
                  <div className="flex flex-col items-center text-center p-8">
                      <Spinner />
                      <p className="mt-4 text-slate-400">Removing object...</p>
                  </div>
                )}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="mt-8 space-y-4">
            <div>
              <label htmlFor="prompt-remover" className="block text-sm font-medium text-slate-300 mb-2">
