...
const Unblurrer: React.FC = () => {
  ...
  useEffect(() => {
    if (chainState && chainState.targetTool === 'unblur') {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!originalImage) {
      setError('Please upload an image to unblur.');
      return;
    }
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      const fullPrompt = "Unblur this image. Enhance its sharpness, clarity, and details to make it look crisp and clear.";
      const newImageBase64 = await editImage(fullPrompt, base64, mimeType);
      setResultImage(`data:${mimeType};base64,${newImageBase64}`);
    } catch (err: any) {
      console.error("Unblur failed:", err);
      setError("Could not unblur the image. This can happen if the image is too blurry or low quality.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="w-full max-w-5xl mx-auto">
      <div className="text-center mb-8">
        <h2 className="text-3xl font-extrabold text-blue-50 tracking-tight">AI Image Unblur</h2>
        <p className="mt-2 text-lg text-blue-300">Fix blurry photos and bring details into focus.</p>
      </div>
        ...
        <div className="bg-blue-900/80 rounded-2xl p-4 sm:p-6 md:p-8 border border-blue-800 shadow-2xl shadow-black/30">
          {!resultImage && !isLoading ? (
             <div className="flex flex-col items-center">
               ...
               <img src={originalImage.url} alt="Original for unblur" className="rounded-lg shadow-lg max-w-full h-auto" />
             </div>
          ) : (
            <div className="animate-fade-in space-y-8">
              ...
              {resultImage && (
                <div>
                  ...
                  <GeneratedImage src={resultImage} alt="Unblurred image" prompt="unblurred-image" />
                </div>
              )}
               {isLoading && (
                  <div className="flex flex-col items-center text-center p-8">
                      <Spinner />
                      <p className="mt-4 text-blue-300">Bringing image into focus...</p>
                  </div>
                )}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="mt-8 space-y-4">
            ...
            <div className="flex flex-col sm:flex-row justify-center items-center gap-4 pt-4">
              <Button type="submit" isLoading={isLoading} className="w-full sm:w-auto" title="Unblur the uploaded image">
                Unblur Image
              </Button>
