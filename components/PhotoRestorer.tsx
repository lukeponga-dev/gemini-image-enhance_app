...
const PhotoRestorer: React.FC = () => {
  ...
  useEffect(() => {
    if (chainState && chainState.targetTool === 'restore') {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto">
      <div className="text-center mb-8">
        <h2 className="text-3xl font-extrabold text-blue-50 tracking-tight">AI Old Photo Restorer</h2>
        <p className="mt-2 text-lg text-blue-300">Bring your old, damaged photos back to life.</p>
      </div>
        ...
        <div className="bg-blue-900/80 rounded-2xl p-4 sm:p-6 md:p-8 border border-blue-800 shadow-2xl shadow-black/30">
          {!resultImage && !isLoading ? (
             <div className="flex flex-col items-center">
               <h3 className="text-xl font-semibold mb-4 text-center text-blue-200">Your Photo</h3>
               <img src={originalImage.url} alt="Original for photo restoration" className="rounded-lg shadow-lg max-w-full h-auto" />
             </div>
          ) : (
              ...
              {resultImage && (
               ...
               {isLoading && (
                  <div className="flex flex-col items-center text-center p-8">
                      <Spinner />
                      <p className="mt-4 text-blue-300">Restoring photo...</p>
                  </div>
                )}
            </div>
          )}
