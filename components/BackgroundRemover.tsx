...
const BackgroundRemover: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (chainedData?.targetTool === 'bg-remover') {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto">
      <div className="text-center mb-8">
        <h2 className="text-3xl font-extrabold text-zinc-50 tracking-tight">AI Background Remover</h2>
        <p className="mt-2 text-lg text-zinc-400">Isolate the subject of your photo with a single click.</p>
      </div>
        ...
        <div className="bg-zinc-900/80 rounded-2xl p-4 sm:p-6 md:p-8 border border-zinc-800 shadow-2xl shadow-black/30">
          {!resultImage && !isLoading ? (
             <div className="flex flex-col items-center">
               ...
               <img src={originalImage.url} alt="Original for background removal" className="rounded-lg shadow-lg max-w-md h-auto" />
             </div>
          ) : (
              ...
              {resultImage && (
               ...
               {isLoading && (
                  <div className="flex flex-col items-center text-center p-8">
                      <Spinner />
                      <p className="mt-4 text-zinc-400">Removing background...</p>
                  </div>
                )}
            </div>
          )}
