...
const ImageComparator: React.FC<ImageComparatorProps> = ({ before, after, beforeAlt = 'Before', afterAlt = 'After' }) => {
  ...
  return (
    <div
      ref={containerRef}
      className="relative w-full max-w-2xl mx-auto aspect-square select-none overflow-hidden rounded-lg shadow-lg bg-gray-900/50"
    >
      <img
        src={before}
        alt={beforeAlt}
      ...
      >
        <div
          className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 h-10 w-10 bg-white/80 rounded-full flex items-center justify-center shadow-lg backdrop-blur-sm cursor-ew-resize border-2 border-white/50 transition-transform duration-200 group-hover:scale-110"
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-blue-600 rotate-90" fill="none" viewBox="0 0 24 24" stroke="currentColor">
